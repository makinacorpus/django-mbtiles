...
urlpatterns = patterns('',
    url(r'^(?P<name>%s)/(?P<z>(\d+|\{z\}))/(?P<x>(\d+|\{x\}))/(?P<y>(\d+|\{y\})).(?P<format>([a-z]+))$' % MBTILES_ID_PATTERN, tile, name="tile"),
    ...
    url(r'^(?P<catalog>%s)/(?P<name>%s)/(?P<z>(\d+|\{z\}))/(?P<x>(\d+|\{x\}))/(?P<y>(\d+|\{y\})).(?P<format>([a-z]+))$' % (MBTILES_CATALOG_PATTERN, MBTILES_ID_PATTERN), tile, name="tile"),
